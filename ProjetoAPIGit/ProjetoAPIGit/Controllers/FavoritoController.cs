...
using ProjetoAPIGit.Context;
using ProjetoAPIGit.Models;

namespace ProjetoAPIGit.Controllers
{
    public class FavoritoController : Controller
    {
        ...
        public ActionResult Favoritar([Bind(Include = "IdFavorito,IdRepositorio,NomeRepositorio,Username")] Favorito favorito)
        {
            ...
            if (fav != default)
            {
                ModelState.AddModelError("Name", "Este repositório já foi favoritado");
            } else
            {
                favorito.IdRepositorio = User.Identity.Name;
                db.tbFavorito.Add(favorito);
                db.SaveChanges();

            }
            return RedirectToAction("ListarFavoritos", "Home", null);
        }
